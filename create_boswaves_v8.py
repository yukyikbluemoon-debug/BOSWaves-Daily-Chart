...
# ⚠️ token ใน .env หรือ GitHub Secrets เท่านั้น
...
#  fallback ใช้ค่า default ถ้า Supabase ไม่พร้อม
# ═══════════════════════════════════════════════════════════════════
import urllib.request, json as _json

def _load_supabase_config() -> dict:
    """อ่าน config และ recipients จาก Supabase"""
    defaults = {
    ...
    try:
        ...
        req = urllib.request.Request(
            f"{SUPABASE_URL}/rest/v1/boswaves_config?id=eq.1&select=*",
            headers=headers)
        with urllib.request.urlopen(req, timeout=10) as r:
            rows = _json.loads(r.read())
        if rows:
            cfg = rows[0]
            defaults["tickers"]         = cfg.get("tickers", defaults["tickers"])
            defaults["rsi_overbought"]  = cfg.get("rsi_overbought", 70)
            defaults["rsi_oversold"]    = cfg.get("rsi_oversold", 30)
            defaults["gemini"]          = cfg.get("gemini", defaults["gemini"])
            defaults["schedules"]       = cfg.get("schedules", [])

        # อ่าน recipients
        req2 = urllib.request.Request(
            f"{SUPABASE_URL}/rest/v1/boswaves_recipients"
            f"?active=eq.true&select=chat_id,name",
            headers=headers)
        with urllib.request.urlopen(req2, timeout=10) as r:
            recips = _json.loads(r.read())
        defaults["chat_ids"] = [r["chat_id"] for r in recips]

    except Exception as e:
        print(f"[WARN] Supabase config load error: {e} — ใช้ค่า default")
    return defaults
...
# guard
if not BOT_TOKEN:
    log.error("TELEGRAM_TOKEN ไม่พบใน .env — ยกเลิก")
    sys.exit(1)
if not CHAT_IDS:
    log.error("ไม่พบ Chat ID (Supabase recipients ว่างเปล่าและไม่มี TELEGRAM_CHAT_ID) — ยกเลิก")
    sys.exit(1)
log.info(f"ผู้รับ Telegram: {len(CHAT_IDS)} คน")
...
def tg_send_photo(image_path: Path, caption: str,
                  chat_ids: list | None = None) -> bool:
    """ส่งรูปไปหลาย Chat ID — คืน True ถ้าส่งสำเร็จอย่างน้อย 1 คน"""
    ids  = chat_ids or CHAT_IDS
    ...
    for cid in ids:
        try:
            ...
            if resp.status_code == 200 and resp.json().get("ok"):
                ...
                log.error(f"  Telegram [{cid}] error: {resp.text}")
        except Exception as e:
            log.error(f"  Telegram [{cid}] exception: {e}")
    return ok_count > 0

def tg_send_text(text: str, chat_ids: list | None = None) -> None:
    """ส่งข้อความไปหลาย Chat ID"""
    ids = chat_ids or CHAT_IDS
...
def _rsi_tag_short(rsi: float) -> str:
    """แปล RSI สั้นๆ สำหรับ Daily caption"""
    if rsi >= RSI_OVERBOUGHT:
        return f"  🔥 RSI {rsi:.0f} ราคาร้อนแรง"
    elif rsi <= RSI_OVERSOLD:
        return f"  💎 RSI {rsi:.0f} ราคาถูกผิดปกติ"
    return ""

def tg_rsi_alert(ticker: str, rsi: float, price: float) -> None:
    """[แบบ A] ส่งแจ้งเตือน RSI แยกทันที ภาษาไทยเข้าใจง่าย"""
    if rsi >= RSI_OVERBOUGHT:
        text = (
            f"⚠️ <b>สัญญาณ {ticker}</b>\n\n"
            f"🔥 <b>ราคาร้อนแรงเกินไปแล้ว!</b>\n"
            f"ตัวเลข RSI อยู่ที่ {rsi:.1f}\n"
            f"(ปกติไม่ควรเกิน {RSI_OVERBOUGHT})\n\n"
            f"ราคาปัจจุบัน: <b>${price:.2f}</b>\n"
            f"👉 ยังไม่ควรรีบซื้อเพิ่ม\n"
            f"   รอให้ราคาเย็นลงก่อนครับ\n\n"
        ...
        text = (
            ...
            f"ตัวเลข RSI อยู่ที่ {rsi:.1f}\n"
            f"(ปกติไม่ควรต่ำกว่า {RSI_OVERSOLD})\n\n"
            f"ราคาปัจจุบัน: <b>${price:.2f}</b>\n"
            f"👉 อาจเป็นจังหวะที่น่าสนใจ\n"
            f"   แต่ควรดูสัญญาณอื่นประกอบด้วยครับ\n\n"
            f"<i>⚠️ ไม่ใช่คำแนะนำทางการเงิน</i>"
        )
    else:
        return  # RSI ปกติ ไม่ต้องแจ้ง
    tg_send_text(text)
...
for ticker in TICKERS:
    ...
    try:
        ...
        # [แบบ A] แจ้งเตือน RSI ทันทีถ้าผิดปกติ
